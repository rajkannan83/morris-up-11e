Survey = new Class({
    ...
    render: function() {
        ...
        if ($$('article.slide-' + this.currentSlide)[0].hasClass('question')) {
            ...
            $$("article.slide-"+this.currentSlide)[0].getElement('p').focus();
    ...
    showResults: function() {
        ...
        this.questions.each(function(item, index){
                ...
                var Column =  new Element('td', {'tabindex': '0', 'html': '<span class="aria-offscreen">original question.</span><span>' + String.from(index+1) + '.  </span>' + item.question, 'class': 'results-question', 'colspan':'4'})
                                .inject(Row);
               ...
               /* Column =  new Element('td',{'html': item.question,'colspan':'3', 'class': 'results-question'})
                ...
                Column =  new Element('th',{'html':"<strong>Response</strong>", 'colspan':'2', 'class': 'first-column-header'})
                                .inject(Row);
                ...
                item.resultLabels.each(function(resultitem, index){
                    ...
                    Column =  new Element('td',{'html':resultitem.label,'colspan':'2', 'class': 'response ' + (selected ? "user-choice" : "other-choice")})
                                    .inject(Row);
                ...
                Column =  new Element('td',{'class': 'spacer', 'colspan': '4'})
                                .inject(Row);
    ...
    createDOM: function() {
        ...
        if (this.data['checkBoxes']) {
            for (var j = 0; j < this.data['checkBoxes'].length; j++) {
                new Element('li', {
                    html: '<input tabindex="0" id="'+ this.options.id +'-'+ j + this.data.checkBoxes[j].label +'" type="checkbox" class="answer" name="question-' + (this.options.id + 1) + '" value="' + this.data.checkBoxes[j].label + '"> <label for="'+ this.options.id +'-'+ j +this.data.checkBoxes[j].label+'">' + this.data.checkBoxes[j].label + '</label>'
                }).inject(option_list);
            }
        }
        else{
            for (var j = 0; j < this.data['radioButtons'].length; j++) {
                new Element('li', {
                     html: '<input tabindex="0" id="'+ this.options.id +'-'+ j + this.data.radioButtons[j].label +'" type="radio" class="answer" name="question-' + (this.options.id + 1) + '" value="' + this.data.radioButtons[j].label + '"><label for="'+ this.options.id +'-'+ j +this.data.radioButtons[j].label+'">' + this.data.radioButtons[j].label + '</label> '
                }).inject(option_list);
            }
        }
    }
